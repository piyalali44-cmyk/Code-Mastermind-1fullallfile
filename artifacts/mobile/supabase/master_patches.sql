...
-- Comprehensive: reapplies ALL columns, indexes, functions, policies, tables.
-- On a fresh DB: run master_patches.sql once via Supabase SQL Editor (or the
-- Management API fallback using SUPABASE_ACCESS_TOKEN); thereafter every API
-- restart self-heals automatically using only the service-role key.
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- ── 5. is_admin() helper (idempotent via CREATE OR REPLACE) ──────────────
  EXECUTE $exec$
    CREATE OR REPLACE FUNCTION public.is_admin()
  ...
  -- ── 6. Admin RLS policies (idempotent: drop then recreate) ────────────────
  DROP POLICY IF EXISTS "Admins manage subscriptions"  ON public.subscriptions;
  CREATE POLICY "Admins manage subscriptions"
    ON public.subscriptions FOR ALL TO authenticated
    USING (public.is_admin()) WITH CHECK (public.is_admin());
  ...
  -- ── 8. Referral functions (idempotent via EXECUTE + CREATE OR REPLACE) ────
  EXECUTE $exec$
    CREATE OR REPLACE FUNCTION public.apply_referral_code(p_code TEXT)
    RETURNS JSONB SECURITY DEFINER SET search_path = public LANGUAGE plpgsql AS $$
    DECLARE
      v_referrer_id UUID; v_caller UUID := auth.uid();
      v_clean_code  TEXT := upper(trim(p_code));
    BEGIN
      ...
      WHERE upper(referral_code) = v_clean_code AND id != v_caller LIMIT 1;
      IF v_referrer_id IS NULL THEN RETURN jsonb_build_object('success',false,'error','invalid_code'); END IF;
      IF EXISTS (SELECT 1 FROM public.referrals WHERE referred_id = v_caller LIMIT 1) THEN
        RETURN jsonb_build_object('success',false,'error','already_used'); END IF;
      INSERT INTO public.referrals (referrer_id,referred_id,code_used,xp_awarded_referrer,xp_awarded_referred)
      VALUES (v_referrer_id,v_caller,v_clean_code,500,100) ON CONFLICT (referred_id) DO NOTHING;
      IF NOT FOUND THEN RETURN jsonb_build_object('success',false,'error','already_used'); END IF;
      INSERT INTO public.user_xp (user_id,total_xp,level,updated_at) VALUES (v_referrer_id,500,2,NOW())
      ON CONFLICT (user_id) DO UPDATE SET
        total_xp=public.user_xp.total_xp+500,
        level=GREATEST(1,FLOOR((public.user_xp.total_xp+500)::FLOAT/500)::INTEGER+1),updated_at=NOW();
      INSERT INTO public.daily_xp_log (user_id,xp_amount,reason) VALUES (v_referrer_id,500,'referral_friend_joined');
      INSERT INTO public.user_xp (user_id,total_xp,level,updated_at) VALUES (v_caller,100,1,NOW())
      ON CONFLICT (user_id) DO UPDATE SET
        total_xp=public.user_xp.total_xp+100,
        level=GREATEST(1,FLOOR((public.user_xp.total_xp+100)::FLOAT/500)::INTEGER+1),updated_at=NOW();
      INSERT INTO public.daily_xp_log (user_id,xp_amount,reason) VALUES (v_caller,100,'referral_bonus');
      RETURN jsonb_build_object('success',true,'xp_bonus',100,'type','referral');
    END; $$
  $exec$;
  ...
  EXECUTE $exec$
    ...
    RETURNS JSONB SECURITY DEFINER SET search_path = public LANGUAGE plpgsql AS $$
    DECLARE
      v_referrer_id UUID; v_clean_code TEXT := upper(trim(p_code));
    BEGIN
      IF p_user_id IS NULL THEN RETURN jsonb_build_object('success',false,'error','not_authenticated'); END IF;
      SELECT id INTO v_referrer_id FROM public.profiles
      WHERE upper(referral_code) = v_clean_code AND id != p_user_id LIMIT 1;
      IF v_referrer_id IS NULL THEN RETURN jsonb_build_object('success',false,'error','invalid_code'); END IF;
      IF EXISTS (SELECT 1 FROM public.referrals WHERE referred_id = p_user_id LIMIT 1) THEN
        RETURN jsonb_build_object('success',false,'error','already_used'); END IF;
      INSERT INTO public.referrals (referrer_id,referred_id,code_used,xp_awarded_referrer,xp_awarded_referred)
      VALUES (v_referrer_id,p_user_id,v_clean_code,500,100) ON CONFLICT (referred_id) DO NOTHING;
      IF NOT FOUND THEN RETURN jsonb_build_object('success',false,'error','already_used'); END IF;
      INSERT INTO public.user_xp (user_id,total_xp,level,updated_at) VALUES (v_referrer_id,500,2,NOW())
      ON CONFLICT (user_id) DO UPDATE SET
        total_xp=public.user_xp.total_xp+500,
        level=GREATEST(1,FLOOR((public.user_xp.total_xp+500)::FLOAT/500)::INTEGER+1),updated_at=NOW();
      INSERT INTO public.daily_xp_log (user_id,xp_amount,reason) VALUES (v_referrer_id,500,'referral_friend_joined');
      INSERT INTO public.user_xp (user_id,total_xp,level,updated_at) VALUES (p_user_id,100,1,NOW())
      ON CONFLICT (user_id) DO UPDATE SET
        total_xp=public.user_xp.total_xp+100,
        level=GREATEST(1,FLOOR((public.user_xp.total_xp+100)::FLOAT/500)::INTEGER+1),updated_at=NOW();
      INSERT INTO public.daily_xp_log (user_id,xp_amount,reason) VALUES (p_user_id,100,'referral_bonus');
      RETURN jsonb_build_object('success',true,'xp_bonus',100,'type','referral');
    END; $$
  $exec$;
  ...
  -- ── 9. Admin award functions ──────────────────────────────────────────────
  EXECUTE $exec$
    CREATE OR REPLACE FUNCTION public.admin_award_xp(p_user_id UUID, p_amount INTEGER, p_reason TEXT DEFAULT 'Admin manual award')
    RETURNS jsonb SECURITY DEFINER SET search_path = public LANGUAGE plpgsql AS $$
    DECLARE v_cur INTEGER; v_new INTEGER; v_level INTEGER;
    BEGIN
      IF NOT public.is_admin() THEN RAISE EXCEPTION 'Admin access required'; END IF;
      SELECT COALESCE(total_xp,0) INTO v_cur FROM public.user_xp WHERE user_id = p_user_id;
      v_new   := GREATEST(0, COALESCE(v_cur,0) + p_amount);
      v_level := GREATEST(1, FLOOR(v_new::FLOAT/500)::INTEGER + 1);
      INSERT INTO public.user_xp (user_id,total_xp,level,updated_at) VALUES (p_user_id,v_new,v_level,NOW())
      ON CONFLICT (user_id) DO UPDATE SET total_xp=EXCLUDED.total_xp,level=EXCLUDED.level,updated_at=EXCLUDED.updated_at;
      INSERT INTO public.daily_xp_log (user_id,xp_amount,reason) VALUES (p_user_id,p_amount,p_reason);
      RETURN jsonb_build_object('total_xp',v_new,'level',v_level);
    END; $$
  $exec$;
  ...
  EXECUTE $exec$
    CREATE OR REPLACE FUNCTION public.admin_award_badge(p_user_id UUID, p_badge_id UUID)
    RETURNS jsonb SECURITY DEFINER SET search_path = public LANGUAGE plpgsql AS $$
    DECLARE v_badge RECORD; v_cur INTEGER; v_new INTEGER; v_level INTEGER; v_rows BIGINT;
    BEGIN
      IF NOT public.is_admin() THEN RAISE EXCEPTION 'Admin access required'; END IF;
      SELECT * INTO v_badge FROM public.badges WHERE id = p_badge_id;
      IF NOT FOUND THEN RAISE EXCEPTION 'Badge not found: %', p_badge_id; END IF;
      INSERT INTO public.user_badges (user_id,badge_id) VALUES (p_user_id,p_badge_id) ON CONFLICT (user_id,badge_id) DO NOTHING;
      GET DIAGNOSTICS v_rows = ROW_COUNT;
      IF v_rows > 0 AND COALESCE(v_badge.xp_reward,0) > 0 THEN
        SELECT COALESCE(total_xp,0) INTO v_cur FROM public.user_xp WHERE user_id = p_user_id;
        v_new   := COALESCE(v_cur,0) + v_badge.xp_reward;
        ...
        INSERT INTO public.user_xp (user_id,total_xp,level,updated_at) VALUES (p_user_id,v_new,v_level,NOW())
        ON CONFLICT (user_id) DO UPDATE SET total_xp=EXCLUDED.total_xp,level=EXCLUDED.level,updated_at=EXCLUDED.updated_at;
        INSERT INTO public.daily_xp_log (user_id,xp_amount,reason) VALUES (p_user_id,v_badge.xp_reward,'Badge awarded: '||v_badge.name);
      END IF;
      RETURN jsonb_build_object('badge_id',p_badge_id,'badge_name',v_badge.name,'xp_reward',COALESCE(v_badge.xp_reward,0),'already_had',(v_rows=0));
    END; $$
  $exec$;
  ...
  EXECUTE $exec$
    CREATE OR REPLACE FUNCTION public.admin_revoke_badge(p_user_id UUID, p_badge_id UUID)
  ...
  -- ── 10. Re-seed badges and settings ──────────────────────────────────────
  INSERT INTO public.badges (slug, name, description, icon, xp_reward) VALUES
    ('hadith_start', 'Hadith Seeker',  'First hadith episode completed',  '📜', 15),
    ('hadith_10',    'Hadith Student', 'Completed 10 hadith episodes',    '📚', 75),
    ('hadith_40',    'Hadith Scholar', 'Completed 40 hadith episodes',    '🏛️', 300)

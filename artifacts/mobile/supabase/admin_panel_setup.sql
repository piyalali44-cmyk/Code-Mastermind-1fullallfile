-- ╔══════════════════════════════════════════════════════════════════════════════╗
-- ║  StayGuided Me — ADMIN PANEL SETUP SQL                                      ║
-- ║                                                                              ║
-- ║  Run this ONCE in: Supabase Dashboard → SQL Editor → New Query              ║
-- ║  Safe to re-run — every statement is idempotent (IF NOT EXISTS / OR REPLACE)║
-- ║                                                                              ║
-- ║  Covers:                                                                     ║
-- ║   • Missing columns (access_tier, image_url, store, push_token, etc.)       ║
-- ║   • Library tables (favourites, bookmarks, downloads, user_settings)         ║
-- ║   • Coupon system (coupon_codes, coupon_redemptions)                         ║
-- ║   • RLS policies for admin access                                            ║
-- ║   • All admin functions (is_admin, award_xp, award_badge, etc.)              ║
-- ║   • Referral functions                                                       ║
-- ║   • Indexes for performance                                                  ║
-- ║   • Badge seeds (Hadith badges)                                              ║
-- ║   • stayguided_apply_patches() RPC for self-healing on API restart           ║
-- ╚══════════════════════════════════════════════════════════════════════════════╝

-- ═══════════════════════════════════════════════════════════════════════════
-- SECTION 1: COLUMN ADDITIONS (all idempotent)
-- ═══════════════════════════════════════════════════════════════════════════

-- Episodes: image_url + access_tier
...
-- Subscriptions: store / product_id / original_transaction_id
...
-- Push campaigns / notifications: image_url
...
-- ── Backfill access_tier from is_premium ────────────────────────────────────
UPDATE public.episodes SET access_tier = 'premium' WHERE is_premium = true AND (access_tier IS NULL OR access_tier = 'free');
UPDATE public.series   SET access_tier = 'premium' WHERE is_premium = true AND (access_tier IS NULL OR access_tier = 'free');
UPDATE public.episodes SET access_tier = 'free'    WHERE access_tier IS NULL;
UPDATE public.series   SET access_tier = 'free'    WHERE access_tier IS NULL;
...
-- ── Normalise referral codes to uppercase ───────────────────────────────────
...
-- ═══════════════════════════════════════════════════════════════════════════
-- SECTION 6: ADMIN RLS POLICIES
-- (Drop-then-recreate so they always reflect the latest rules)
-- ═══════════════════════════════════════════════════════════════════════════
...
-- Referrals visibility for admins
...
BEGIN
  ...
  WHERE upper(referral_code) = v_clean_code AND id != v_caller
  LIMIT 1;
...
BEGIN
  ...
  SELECT id INTO v_referrer_id
  FROM public.profiles
  WHERE upper(referral_code) = v_clean_code AND id != p_user_id
  LIMIT 1;
  IF v_referrer_id IS NULL THEN
    RETURN jsonb_build_object('success', false, 'error', 'invalid_code');
  END IF;
...
-- ═══════════════════════════════════════════════════════════════════════════
-- SECTION 9: ADMIN AWARD FUNCTIONS (XP / Badge)
-- ═══════════════════════════════════════════════════════════════════════════
...
LANGUAGE plpgsql AS $$
DECLARE
  v_cur INTEGER; v_new INTEGER; v_level INTEGER;
BEGIN
...
LANGUAGE plpgsql AS $$
DECLARE
  v_badge RECORD; v_cur INTEGER; v_new INTEGER; v_level INTEGER; v_rows BIGINT;
BEGIN
  IF NOT public.is_admin() THEN RAISE EXCEPTION 'Admin access required'; END IF;
  ...
  END IF;
  RETURN jsonb_build_object('badge_id', p_badge_id, 'badge_name', v_badge.name,
    'xp_reward', COALESCE(v_badge.xp_reward, 0), 'already_had', (v_rows = 0));
END;
$$;
...
-- ═══════════════════════════════════════════════════════════════════════════
-- SECTION 10: REDEEM_CODE FUNCTION (coupons + referrals unified)
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  LIMIT 1;
  IF FOUND THEN
    IF v_coupon.max_total_uses IS NOT NULL AND v_coupon.redemption_count >= v_coupon.max_total_uses THEN
      RETURN jsonb_build_object('success', FALSE, 'error', 'code_exhausted');
    END IF;
    SELECT COUNT(*) INTO v_redemptions
    FROM public.coupon_redemptions WHERE coupon_id = v_coupon.id AND user_id = p_user_id;
    ...
    VALUES (v_coupon.id, p_user_id) ON CONFLICT (coupon_id, user_id) DO NOTHING;
    UPDATE public.coupon_codes SET redemption_count = redemption_count + 1, updated_at = NOW()
    ...
    ELSIF v_coupon.coupon_type IN ('free_days','free_period') AND COALESCE(v_coupon.free_days, 0) > 0 THEN
      ...
      UPDATE public.profiles SET subscription_tier = 'premium', subscription_expires_at = v_expires
      WHERE id = p_user_id;
    ...
    END IF;
  END IF;
  -- Fallback: try as referral code
  RETURN public.process_referral_by_id(p_code, p_user_id);
END;
$$;
...
-- ═══════════════════════════════════════════════════════════════════════════
-- SECTION 11: CHECK_AND_AWARD_BADGES (with Hadith support)
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
-- SECTION 13: APP SETTINGS SEEDS
-- ═══════════════════════════════════════════════════════════════════════════

INSERT INTO public.app_settings (key, value, description, type) VALUES
  ('lifetime_price_usd', '49.99', 'Lifetime subscription price (USD)', 'number')
ON CONFLICT (key) DO NOTHING;


-- ═══════════════════════════════════════════════════════════════════════════
-- SECTION 14: stayguided_apply_patches() — SELF-HEALING RPC
-- The API server calls this on every restart (service-role key).
-- After running THIS file once, no further manual SQL is needed.
-- ═══════════════════════════════════════════════════════════════════════════
...
SELECT 'admin_panel_setup.sql applied successfully ✓' AS result;
